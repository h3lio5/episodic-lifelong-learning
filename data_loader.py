...
class DataSet(data.Dataset):
    ...
    def __getitem__(self,index):
        ...
        # 1280 = 128 (seq length) * 10(upper limit of assumed average word size including spaces on both sides)
        content = self.content[index][:1280]
    ...
    def _add_spl_ids_and_pad(self,input_ids,maxlen=128):
        ...
        if len(input_ids) > maxlen-2:
            input_ids = [self.tokenizer.cls_token_id] + input_ids[:maxlen-2] + [self.tokenizer.sep_token_id]
            return input_ids
